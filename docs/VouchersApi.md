...
namespace Example
{
    public class CreateVoucherExample
    {
        public void main()
        {
            ...
            var storeId = new List<int?>(); // List<int?> | Store list
            var voucher = new CreateVoucher(); // CreateVoucher | Voucher Details

            try
            {
                // [PRIVATE API] Create voucher
                RestApiResultVoucherWithStats result = apiInstance.CreateVoucher(appId, storeId, voucher);
 ...
 **storeId** | [**List&lt;int?&gt;**](int?.md)| Store list | 
