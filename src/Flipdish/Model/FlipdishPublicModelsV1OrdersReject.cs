/* 
 * Flipdish Open API v1.0
 *
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v1.0
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System.ComponentModel.DataAnnotations;
using SwaggerDateConverter = Flipdish.Client.SwaggerDateConverter;

namespace Flipdish.Model
{
    /// <summary>
    /// FlipdishPublicModelsV1OrdersReject
    /// </summary>
    [DataContract]
    public partial class FlipdishPublicModelsV1OrdersReject :  IEquatable<FlipdishPublicModelsV1OrdersReject>, IValidatableObject
    {
        /// <summary>
        /// Gets or Sets RejectReason
        /// </summary>
        [JsonConverter(typeof(StringEnumConverter))]
        public enum RejectReasonEnum
        {
            
            /// <summary>
            /// Enum TooBusy for "TooBusy"
            /// </summary>
            [EnumMember(Value = "TooBusy")]
            TooBusy,
            
            /// <summary>
            /// Enum FoodUnavailable for "FoodUnavailable"
            /// </summary>
            [EnumMember(Value = "FoodUnavailable")]
            FoodUnavailable,
            
            /// <summary>
            /// Enum UnableToDeliver for "UnableToDeliver"
            /// </summary>
            [EnumMember(Value = "UnableToDeliver")]
            UnableToDeliver,
            
            /// <summary>
            /// Enum UnknownAddress for "UnknownAddress"
            /// </summary>
            [EnumMember(Value = "UnknownAddress")]
            UnknownAddress,
            
            /// <summary>
            /// Enum UnknownReason for "UnknownReason"
            /// </summary>
            [EnumMember(Value = "UnknownReason")]
            UnknownReason,
            
            /// <summary>
            /// Enum TooSoon for "TooSoon"
            /// </summary>
            [EnumMember(Value = "TooSoon")]
            TooSoon,
            
            /// <summary>
            /// Enum TimeUnavailable for "TimeUnavailable"
            /// </summary>
            [EnumMember(Value = "TimeUnavailable")]
            TimeUnavailable,
            
            /// <summary>
            /// Enum DontDeliverToArea for "DontDeliverToArea"
            /// </summary>
            [EnumMember(Value = "DontDeliverToArea")]
            DontDeliverToArea,
            
            /// <summary>
            /// Enum StoreUncontactable for "StoreUncontactable"
            /// </summary>
            [EnumMember(Value = "StoreUncontactable")]
            StoreUncontactable
        }

        /// <summary>
        /// Gets or Sets RejectReason
        /// </summary>
        [DataMember(Name="rejectReason", EmitDefaultValue=false)]
        public RejectReasonEnum? RejectReason { get; set; }
        /// <summary>
        /// Initializes a new instance of the <see cref="FlipdishPublicModelsV1OrdersReject" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected FlipdishPublicModelsV1OrdersReject() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="FlipdishPublicModelsV1OrdersReject" /> class.
        /// </summary>
        /// <param name="RejectReason">RejectReason (required).</param>
        public FlipdishPublicModelsV1OrdersReject(RejectReasonEnum? RejectReason = default(RejectReasonEnum?))
        {
            // to ensure "RejectReason" is required (not null)
            if (RejectReason == null)
            {
                throw new InvalidDataException("RejectReason is a required property for FlipdishPublicModelsV1OrdersReject and cannot be null");
            }
            else
            {
                this.RejectReason = RejectReason;
            }
        }
        

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class FlipdishPublicModelsV1OrdersReject {\n");
            sb.Append("  RejectReason: ").Append(RejectReason).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            // credit: http://stackoverflow.com/a/10454552/677735
            return this.Equals(obj as FlipdishPublicModelsV1OrdersReject);
        }

        /// <summary>
        /// Returns true if FlipdishPublicModelsV1OrdersReject instances are equal
        /// </summary>
        /// <param name="other">Instance of FlipdishPublicModelsV1OrdersReject to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(FlipdishPublicModelsV1OrdersReject other)
        {
            // credit: http://stackoverflow.com/a/10454552/677735
            if (other == null)
                return false;

            return 
                (
                    this.RejectReason == other.RejectReason ||
                    this.RejectReason != null &&
                    this.RejectReason.Equals(other.RejectReason)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            // credit: http://stackoverflow.com/a/263416/677735
            unchecked // Overflow is fine, just wrap
            {
                int hash = 41;
                // Suitable nullity checks etc, of course :)
                if (this.RejectReason != null)
                    hash = hash * 59 + this.RejectReason.GetHashCode();
                return hash;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
